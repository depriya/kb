...
# Toolchain Scenario: Virtual ECU Builder With Orchestration Blueprint
...
This directory demonstrates an end-to-end scenario where a Yocto OS image is built and deployed onto a new vECU (Virtual Electronic Control Unit), then the [orchestration blueprint](https://github.com/eclipse-sdv-blueprints/software-orchestration/blob/main/README.md) is installed on the vECU. This scenario is based off of the [Virtual ECU Builder Scenario](../vecu_builder_guide/README.md). More information about blueprints can be found on the [Eclipse SDV Blueprints](https://github.com/eclipse-sdv-blueprints) github organization page.
...
1. Downloads installation files for the [Eclipse Software Orchestration Blueprint](https://github.com/eclipse-sdv-blueprints/software-orchestration).
1. Runs installation script on the vECU.
...
> - In this implementation, the VM password is passed in plain text when copying the installation files to the vECU VM and when running the installation script on the vECU VM. For better security in production, use authentication with an [identity file](https://www.ssh.com/academy/ssh/identity-key).
...
Describes a software stack. Multiple software stacks can be declared. The main sections are the `os` and `middleware`. Middleware describes the software that will be installed on the vECU. `orchestration_blueprint` is an example of a middleware that will be installed on the vECU. It is a set of files that will be copied fro the host to the vECU. `orchestrator` is the the orchestration service that will be selected in the blueprint. `install_script` is the name of the installation script used to download and build the [orchestration blueprint](https://github.com/eclipse-sdv-blueprints/software-orchestration). `install_files` includes the fields `src_dir` and `dst_dir`. `src_dir` is the path to the directory containing files to be installed on the host.  `dst_dir` is the path to the directory on the vECU.
...
`copy_file_to_vecu` - to copy file(s) from the host to the vECU. It describes the configuration of the functionality to copy file(s) from the host to the vECU once the vECU is deployed.

`execute_command_on_vecu` - to run commands on the vECU via ssh. It describes the configuration of the functionality to run bash commands via ssh on the vECU once the vECU is deployed.
...
1. Copy installation files from the host to the vECU (see the `copy_file_to_vecu` target).
1. Run commands on the vECU (see the `execute_command_on_vecu` target).
...
1. Execute the `vecu.yaml` metamodel (with optional overrides) to generate the files needed to run the two Symphony campaigns. Example:
    ...
    ./toolchain_cli.py metamodel execute \
      --from-file /workspaces/sdv-toolchain/scenarios/orchestration_blueprint_guide/vecu.yaml \
      --override /workspaces/sdv-toolchain/scenarios/orchestration_blueprint_guide/vecu_user_overrides.yaml \

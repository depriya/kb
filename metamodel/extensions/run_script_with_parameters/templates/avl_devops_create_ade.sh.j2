...
COMPUTE_GALLERY="Computegalleryid"           
...
suffix=$(echo $config | jq -r '.suffix')
...
ENVIRONMENT_NAME="${resource_name_primary_prefix}-${resource_name_secondary_prefix}-${resource_name_customer_short}-${customer_OEM_suffix}-p-${project_name}-${environment_stage_short}-vmss-${suffix}"
echo "setting vmss name $ENVIRONMENT_NAME"
PROJECT="${resource_name_primary_prefix}-${resource_name_secondary_prefix}-${resource_name_customer_short}-${customer_OEM_suffix}-p-${project_name}-${suffix}"
...
command="az devcenter dev environment create \
            ...
            --parameters '{\"customerOEMsuffix\":\"${customer_OEM_suffix}\",\"admin_username\":\"${ADMIN_USER}\",\"environmentStage\":\"${environment_stage_short}\",\"vmss_uniquesuffix\":\"${suffix}\",\"projectname\":\"${project_name}\"}'"
execute_command_exit_on_failure "$command" _command_output _command_status

...
//}
 ...
 extension {
  ...
  settings = jsonencode({
    "commandToExecute" : "powershell.exe -ExecutionPolicy Unrestricted -Command \"New-LocalUser -Name TestEngineer -Password (ConvertTo-SecureString -AsPlainText '${azurerm_key_vault_secret.local_user_password_secret.value}' -Force) -PasswordNeverExpires:$false; Add-LocalGroupMember -Group 'Remote Desktop Users' -Member TestEngineer\""
